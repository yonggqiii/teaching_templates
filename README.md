...
*Use the `\cq` and `\csq` commands sparingly*. They can be used for example in answer booklets where you have MCQ bubbles from 1 to 10, and want to start an answer box for a written question starting at Question 11. Use `\cq` for question 11, then you can use the `\q` and `\sq` commands afterwards.
...
In the future, users will be able to customize the automatically generated instructions.
...
**Question 1** [1 mark]. *Answer*: **(B)**. This is an answer.
